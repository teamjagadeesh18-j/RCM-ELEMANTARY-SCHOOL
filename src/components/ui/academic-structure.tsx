...
export default function AcademicStructure({
  ...
  return (
    <section id="academics" className="py-20 md:py-28 px-4 sm:px-6 md:px-8 bg-[#f8f9fb] border-t border-[#d4e0ed] font-body relative overflow-hidden">
      <div 
        className="absolute inset-0 pointer-events-none"
        style={{
          background: `radial-gradient(circle at 50% 0%, rgba(0, 107, 255, 0.05), transparent 70%)`
        }}
      />
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14 text-left">
          <div className="max-w-2xl">
            ...
            <TextEffect 
              ...
              speedReveal={1.1} 
              speedSegment={0.3} 
              className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#0b3558] tracking-tight leading-tight font-heading"
            >
              {title}
            </TextEffect>
          </div>
          ...
          <div className="max-w-xl md:text-right">
            {subtitle && (
              <TextEffect 
                ...
                speedReveal={1.1} 
                speedSegment={0.3} 
                className="text-lg md:text-xl font-bold text-[#0b3558]/90 font-body mb-2"
              >
                {subtitle}
              </TextEffect>
            )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 items-stretch">
          {cards.map((card, index) => {
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    boxShadow: `0 16px 36px -10px rgba(0, 107, 255, 0.4), inset 0 1px 0 rgba(255,255,255,0.3)`
                  }}
                >
            ...
            return (
              <div
                key={index}
                className={`bg-white border border-[#d4e0ed] rounded-2xl p-6 flex flex-col justify-between text-[#0b3558] transition-all duration-300 transform hover:-translate-y-1 relative overflow-hidden group ${isFirst ? 'ring-1 ring-slate-200/80 shadow-md' : 'shadow-xs hover:shadow-md'}`}
                style={{
                  boxShadow: isFirst ? `0 10px 25px -8px rgba(0, 107, 255, 0.12)` : `0 4px 20px -4px rgba(0,0,0,0.05)`
                }}
              >
                {isFirst && (
                  <div 
                ...
                <div>
                  <span 
                    ...
                    style={{
                      ...
                      color: "#006bff",
                      borderColor: "rgba(0, 107, 255, 0.15)"
                    }}
                  >
                    {card.tag}
                  </span>
                  <TextEffect 
                    ...
                    speedReveal={1.1} 
                    speedSegment={0.3} 
                    className="text-lg font-bold text-[#0b3558] font-heading mb-2.5 leading-snug"
                  >
                    {card.title}
                  </TextEffect>
                  <TextEffect 
                ...
                <a
                  href="#connect"
                  className="inline-flex items-center justify-between text-xs font-bold transition-all duration-200 font-body mt-auto px-4 py-2.5 rounded-2xl bg-slate-50 border border-slate-200/80 text-[#0b3558] hover:bg-slate-100 hover:border-slate-300 shadow-xs active:scale-[0.98] group/btn"
                >
