...
function __OriginkitBase_StarfieldButton(props: StarfieldButtonProps) {
    const {
        ...
        glow = { size: 16, color: "#4795ff", opacity: 100 },
        stroke = {
            size: 96,
            color: "#4795ff",
            count: 1,
        ...
        pixel = { size: 4, color: "#4795ff", density: 50, brightness: 100 },
        link = "",
    ...
    const fill = colors?.fill ?? fillProp ?? "#006bff"
    const textColor = colors?.textColor ?? textColorProp ?? "#FFFFFF"

    const {
        color: glowColor = "#4795ff",
        size: glowSize = 16,
    ...
    const draw = (ctx: CanvasRenderingContext2D) => {
        ...
        for (let k = 0; k < cells.length; k++) {
            ...
            if (a > 1) a = 1
            ctx.globalAlpha = a
            ctx.fillRect(p.cx - off, p.cx - off, dot, dot)
        }
        ctx.globalAlpha = 1
